 ...
 * signs in with, a vote a member cast on a poll. Those are not for the cleanup to remove at any prompting, and they
 ...
 * for: they cannot be reached from anywhere in the site, so the activity appears in the skipped column of every nightly
...
final readonly class StaleRevisionDeletionBlock
{
    private function __construct(
        /** Why the aggregate has to be kept, phrased to follow "kept because" in a log line. */
        public string $reason,
        /** Whether a run given `--force` may remove the aggregate anyway. */
        public bool $forceable,
    ) {
    }

    /**
     * An objection no run overrules, because removing the aggregate would also delete something that is not the
     * cleanup's to remove.
     */
