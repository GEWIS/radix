...
final readonly class AlbumAdminService
{
    ...
    public function deletePhotos(array $photos): void
    {
        ...
        foreach ($photos as $photo) {
            $paths[] = $photo->path;
            $albumIds[intval($photo->album->getId())] = true;
    ...
    /**
     * Write an album, whether it is new or is one being renamed or re-parented. Persisting something the entity
     * manager is already tracking does nothing, so the two cases do not need to be told apart.
     */
