 ...
 * {@see \App\EventListener\User\MfaEnforcementBootListener} before the security stack first resolves a user's roles.
 *
 * Exists because {@see \App\Entity\User\User::getRoles()} cannot constructor-inject services (Doctrine entities are
 * plain objects), but it still has to decide whether to strip `ROLE_ADMIN` / `ROLE_BOARD` for in-scope users who have
