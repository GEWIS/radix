...
class SecurityLogRepository extends ServiceEntityRepository
{
    ...
    public function append(SecurityLog $log): void
    {
        ...
        $this->getEntityManager()->getConnection()->insert(
            ...
            [
                $metadata->getColumnName('occurredAt') => $log->occurredAt,
                $metadata->getColumnName('event') => $log->event->value,
                $metadata->getColumnName('userIdentifier') => $log->userIdentifier,
                $metadata->getColumnName('firewallName') => $log->firewallName,
                $metadata->getColumnName('actorIdentifier') => $log->actorIdentifier,
                $metadata->getColumnName('ipAddress') => $log->ipAddress,
                $metadata->getColumnName('browser') => $log->browser,
                $metadata->getColumnName('operatingSystem') => $log->operatingSystem,
                $metadata->getColumnName('requestId') => $log->requestId,
