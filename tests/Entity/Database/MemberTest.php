...
class MemberTest extends TestCase
{
    ...
    public static function firstMembershipsAndGenerations(): array
    {
        return [
            ...
            'joined in the second half of the association year' => [
                '2027-02-01',
                2026,
            ],
            'joined on the rollover' => [
                '2026-07-01',
                2026,
            ],
        ];
    ...
    public function testEndsMembershipOnTheLastFormalMembership(): void
    {
        ...
        self::assertSame(
            '2027-07-01',
            $member->getMembershipEndDate()?->format('Y-m-d'),
        );
        self::assertSame(
            '2028-07-01',
            $member->getExpiration()->format('Y-m-d'),
        );
    }
    ...
    public function testFallsBackOnASentinelForSomeoneWhoWasNeverAMember(): void
    {
        ...
        self::assertSame(
            '0001-01-01',
            $member->getMembershipEndsOn()->format('Y-m-d'),
        );
    }
    ...
    public function testKnowsWhichMembershipIsRunningRightNow(): void
    {
        ...
        self::assertSame(
            $current,
            $member->getCurrentMembership(),
        );
        self::assertSame(
            $current,
            $member->getCurrentOrLastMembership(),
        );
        self::assertSame(
            $current,
            $member->getLastMembership(),
        );
    }
