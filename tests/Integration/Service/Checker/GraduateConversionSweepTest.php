...
final class GraduateConversionSweepTest extends DatabaseTestCase
{
    ...
    public function testWritesToAMemberWhoseMembershipIsAboutToRunOut(): void
    {
        ...
        $member = $link->member;
        $email = $this->emailTo((string) $member->email);
        ...
        self::assertSame(
            $link->id,
            self::getContainer()->get(ActionLinkService::class)->resolveGraduateConversion($matches[1])?->id,
        );
    }
    ...
    public function testLeavesAMembershipThatEndedLongAgoAlone(): void
    {
        ...
        foreach ($found as $member) {
            ...
            self::assertGreaterThanOrEqual(
                new DateTimeImmutable()->modify('-' . GraduateConversionLink::GRACE_DAYS . ' days')->setTime(
                    0,
                    0,
                ),
                $membership->endDate,
