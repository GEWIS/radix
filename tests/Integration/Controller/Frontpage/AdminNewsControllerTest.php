...
final class AdminNewsControllerTest extends DatabaseTestCase
{
    public function testANewsItemIsWrittenChangedAndTakenDown(): void
    {
        ...
        $this->controller()->create($this->form([
            ...
            'title' => [
                'valueNL' => 'Iets nieuws',
                'valueEN' => 'Something new',
            ],
            'content' => [
                'valueNL' => 'Er is iets gebeurd.',
                'valueEN' => 'Something happened.',
            ],
        ]));
     ...
     *
     * @param array<string, array<string, string|null>|string> $fields
     */
    ...
    /**
     * The item whose English title reads as given. The title is a localised text of its own now, so this is a join
     * rather than a column match.
     */
    private function anItemTitled(string $englishTitle): NewsItem
    {
        $item = $this->repository()->createQueryBuilder('newsItem')
            ->innerJoin(
                'newsItem.title',
                'title',
            )
            ->andWhere('title.valueEN = :englishTitle')
            ->setParameter(
                'englishTitle',
                $englishTitle,
            )
