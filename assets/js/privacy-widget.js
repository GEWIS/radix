/*
 * Wrapped in a function so this file can run more than once in the same document. Turbo Drive re-executes the body
 * scripts of every page it renders, and the top-level `let` declarations this started with threw a redeclaration
 * error on the second run, which stopped the file before it bound anything.
 *
 * Running again is also what makes it correct: the queries below resolve against the body that was just rendered.
 */
(() => {
    // Important elements
    ...
    // A layout that does not include the widget leaves every lookup above null, and each handler assumes otherwise.
    if (
        null === privacy_widget
        || null === privacy_widget_button
        || null === privacy_widget_dismiss
        || null === privacy_widget_checkbox
    ) {
        return;
    }
